...
public class StringMatchesConditional extends ConditionalElement {
    public String matchWith;

    public StringMatchesConditional(String id, JsonObject object) {
        super(id, object);
        ...
        this.matchWith = Text.translatable(object.get("regex").getAsString()).getLiteralString();
    ...
    public JsonObject evaluate() {
        if (getVariable().getType() == Variable.VariableType.TEXT) return ((Variable<Text>) getVariable()).get().getLiteralString().matches(matchWith) ? this.output1: this.output2;
        else return ((String) getVariable().get()).matches(matchWith) ? this.output1 : this.output2;
    }
}
